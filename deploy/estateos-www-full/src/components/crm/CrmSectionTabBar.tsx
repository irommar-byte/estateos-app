...
export default function CrmSectionTabBar({
  ...
  return (
    <div className="relative z-20 mb-8 sm:mb-10 w-full">
      ...
      >
        <LayoutGroup id="crm-section-tabs">
          <div
            ...
            className="mx-auto flex w-max min-w-full items-center justify-start gap-1 rounded-full border border-[var(--eos-border)] bg-[#111] p-1.5 shadow-[inset_0_2px_10px_rgba(0,0,0,0.8)] sm:min-w-0 sm:justify-center sm:gap-1.5 md:inline-flex md:w-auto"
          >
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative shrink-0 rounded-full px-3.5 py-2.5 text-[9px] font-black uppercase tracking-[0.14em] transition-colors sm:px-4 sm:py-3 sm:text-[10px] sm:tracking-[0.16em] md:px-7 md:text-xs md:tracking-[0.18em] ${
                    isActive ? 'text-black' : 'text-[var(--eos-subtle)] hover:text-white/85'
                  }`}
                >
                  {isActive ? (
                    <motion.span
