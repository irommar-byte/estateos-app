...
export default function SignatureHighlights() {
  ...
  return (
    <section className="premium-home-surface relative overflow-hidden border-t border-[var(--eos-border)] bg-[var(--eos-bg-elevated)] py-20 sm:py-28">
      <div
        aria-hidden
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="mt-4 text-3xl font-semibold tracking-tight text-[var(--eos-text)] sm:text-4xl">
            {dict.highlights.title}
          </h2>
          <p className="mt-5 text-base leading-relaxed text-[var(--eos-muted)]">{dict.highlights.body}</p>
        </motion.div>
        ...
        >
          {items.map((item) => (
            <motion.article
              ...
              className="eos-surface-card group relative overflow-hidden rounded-[2.5rem] border border-[var(--eos-border)] bg-[var(--eos-card)] p-8 shadow-[var(--eos-shadow-soft)] transition-all hover:border-emerald-500/25"
            >
              <div
                aria-hidden
              ...
              <h3 className="mt-6 text-xl font-semibold text-[var(--eos-text)]">{item.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-[var(--eos-muted)]">{item.body}</p>
            </motion.article>
          ))}
        </motion.div>
      </div>
