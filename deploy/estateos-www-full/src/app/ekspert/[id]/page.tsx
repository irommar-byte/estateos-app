...
import { getExpertProfileDictionary } from "@/i18n/expertProfileDictionary";
...
export default function ExpertProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ed = getExpertProfileDictionary(locale);
  ...
  if (data.error) return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center gap-6">
       ...
       <h1 className="text-3xl font-black">{ed.notFound}</h1>
       <Link href="/eksperci" className="px-6 py-3 bg-white/10 hover:bg-white/20 rounded-full text-xs font-bold uppercase tracking-widest transition-colors">{ed.backToCatalog}</Link>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white pt-32 pb-40 relative selection:bg-orange-500 selection:text-black">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <Link href="/eksperci" className="text-white/40 hover:text-white mb-8 inline-block text-[10px] uppercase tracking-widest font-bold transition-colors">← {ed.backToCatalog}</Link>
        ...
        <div className="bg-[#0a0a0a] border border-white/10 rounded-[3rem] p-8 md:p-12 shadow-2xl mb-12 flex flex-col md:flex-row items-center md:items-start gap-10 relative overflow-hidden">
           ...
           <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">{ed.verifiedAgent}</span>
              </div>
        ...
        <h2 className="text-2xl font-black tracking-tight mb-6">{ed.portfolioTitle}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {data.offers?.length === 0 ? <p className="text-white/40">{ed.noOffers}</p> : data.offers?.map((o: any) => (
        ...
        {/* OPINIE */}
        <h2 className="text-2xl font-black tracking-tight mb-6">{ed.reviewsTitle}</h2>
        <div className="space-y-4">
           {data.reviews?.length === 0 ? <p className="text-white/40">{ed.noReviews}</p> : data.reviews?.map((r: any) => (
             <div key={r.id} className="bg-[#111] border border-white/5 p-6 rounded-[2rem] flex flex-col gap-3">
                ...
                <p className="text-white/80 text-sm leading-relaxed">{r.comment || ed.defaultReviewComment}</p>
