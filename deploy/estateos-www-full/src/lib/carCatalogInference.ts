...
export function defaultDoorCountForBody(bodyType: string) {
  ...
  if (normalized.includes("coupe") || normalized.includes("coupé") || normalized.includes("kabrio")) return "4";
...
export function pickDoorCountOption(options: CatalogOption[], bodyType: string) {
  if (!options.length) return null;
