...
export const MAX_ACQUISITION_DOCUMENT_BYTES = 40 * 1024 * 1024;
...
function sniffMime(buffer: Buffer, declared: string, fileName: string): string | null {
  ...
  if (declaredTrim === "model/vnd.usdz+zip" || lower.endsWith(".usdz")) {
...
> {
  ...
  if (params.buffer.length > MAX_ACQUISITION_DOCUMENT_BYTES) {
    return { ok: false, status: 413, error: "Plik jest za duży (max 40 MB)." };
  }
  ...
  if (!mime) {
    return { ok: false, status: 415, error: "Dozwolone: PDF, JPG, PNG, WEBP lub USDZ." };
  }
