/**
 * Parser wklejki maila Otodom / OLX — Numer w biurze = offerId EstateOS.
 * Nie mylić z portalowym ID ogłoszenia (np. 68374571).
 */
...
export type ParsedPortalLead = {
  ...
  phone: string | null;
  message: string | null;
  /** ID oferty EstateOS (= „Numer w biurze”). */
...
export function parseOtodomLeadEmail(raw: string): ParsedPortalLead {
  ...
  const titleMatch =
    text.match(/(?:OKAZJA!|[A-ZĄĆĘŁŃÓŚŹŻ].{10,120}?)\s*(?:\n|,)\s*(?:Warszawa|Kraków|Wrocław|Poznań|Gdańsk|[A-Z])/i) ||
    text.match(/Oferta\s*[:：]\s*([^\n]+)/i);
  const rawTitle = titleMatch ? titleMatch[0].split('\n')[0].trim().slice(0, 160) : null;
