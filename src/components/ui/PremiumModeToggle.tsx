...
import { Building, Wallet, Briefcase, User } from 'lucide-react';

export default function PremiumModeToggle({ currentUser }: { currentUser?: any }) {
  const { mode, selectMode } = useUserMode();
  const isPartnerPlan = currentUser?.planType === 'AGENCY';

  // Brak zalogowanego użytkownika – nie pokazujemy przełącznika.
        ...
        {!isPartnerPlan ? (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center gap-1.5 sm:gap-2">
              ...
              <span className="whitespace-nowrap drop-shadow-md">Osoba Prywatna</span>
            </div>
          </button>
        ) : (
          <>
