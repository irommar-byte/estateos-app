...
export default function PriceReductionPreview({ listPricePln, draftPriceRaw }: Props) {
  const draft = parseDraftPrice(draftPriceRaw);
  const discountPercent = useMemo(
    () => (draft > 0 && listPricePln > 0 ? computePriceDiscountPercent(listPricePln, draft) : null),
    [draft, listPricePln],
  );
