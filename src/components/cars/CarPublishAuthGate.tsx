...
export default function CarPublishAuthGate({ open, onClose, onAuthenticated }: CarPublishAuthGateProps) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/75 p-0 sm:items-center sm:p-4">
      <div className="relative flex max-h-[100dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-[1.75rem] border border-sky-400/25 bg-[var(--eos-card)] shadow-2xl sm:max-h-[92dvh] sm:rounded-3xl">
        <button
          type="button"
          onClick={onClose}
          disabled={loading}
        ...
        <div className="shrink-0 border-b border-[var(--eos-border)] bg-gradient-to-r from-sky-500/[0.08] via-transparent to-cyan-500/[0.05] px-5 py-5 sm:px-6">
          ...
          <h2 className="mt-2 pr-8 text-xl font-semibold tracking-tight sm:text-2xl">
            Ostatni krok — konto do publikacji
          </h2>
          <p className="mt-2 text-sm text-[var(--eos-muted)]">
            Załóż konto lub zaloguj się, aby opublikować ogłoszenie, otrzymywać powiadomienia i edytować zdjęcia w
            każdej chwili.
          </p>
        </div>
          ...
          {tab === "register" ? (
            <form onSubmit={handleRegister} className="grid gap-3">
              ...
              <label className="flex items-start gap-2 text-xs text-[var(--eos-muted)]">
                ...
                />
                <span>
                  Akceptuję regulamin EstateOS i wyrażam zgodę na powiadomienia o zapytaniach kupujących.
                </span>
              </label>
              <button
                type="submit"
                disabled={loading}
                className="mt-2 flex w-full items-center justify-center gap-2 rounded-full border border-sky-400/40 bg-sky-500/15 px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-sky-300 disabled:opacity-60"
              >
            ...
            <form onSubmit={handleVerifyOtp} className="grid gap-3">
              <p className="text-sm text-[var(--eos-muted)]">
                Wpisz kod SMS wysłany na numer powiązany z kontem.
              </p>
              <label className="grid gap-1 text-sm">
                ...
                <input
                  ...
                  className="rounded-xl border border-[var(--eos-border)] bg-[var(--eos-surface)] px-3 py-3 text-center text-2xl font-black tracking-[0.3em] outline-none focus:border-sky-400/50"
                  maxLength={6}
                  required
                />
              </label>
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-full border border-sky-400/40 bg-sky-500/15 px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-sky-300 disabled:opacity-60"
              >
                {loading ? <Loader2 size={16} className="animate-spin" /> : null}
