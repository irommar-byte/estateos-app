...
export default function HeroDepthEffect() {
  ...
  const trackHomeCta = (event: HomeCtaAnalyticsEvent, ctaId: HomeCtaId) => {
    ...
    // Re-używamy istniejący tracker endpoint (path) bez zmiany backend contract.
    fetch("/api/track", {
      method: "POST",
  ...
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-black h-[92svh] sm:h-[100dvh]">
      <div className="sticky top-0 h-full w-full flex flex-col items-center justify-start pt-[calc(74px+2vh)] sm:pt-[calc(80px+3vh)] overflow-hidden">
        ...
        <motion.div style={{ y: textY }} className="relative z-10 text-center flex flex-col items-center justify-center w-full px-4">
        ...
        <motion.div 
          ...
          className="absolute bottom-[16vh] md:bottom-[13vh] left-1/2 -translate-x-1/2 z-30 flex flex-col items-center cursor-pointer pointer-events-auto w-max"
        >
          <div 
            onClick={() => openHomeCta("BUY")}
        ...
        <motion.div 
          ...
          className="absolute bottom-10 md:bottom-10 w-full px-4 md:px-12 flex justify-between z-30 pointer-events-auto"
        >
          <div
            onClick={() => openHomeCta("INVESTOR")}
            className="cursor-pointer flex items-center gap-1 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-[#111111]/80 backdrop-blur-md border border-white/10 text-white/50 hover:text-white transition-colors duration-300 group shadow-lg"
          >
