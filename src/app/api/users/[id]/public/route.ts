...
async function loadReviews(revieweeId: number) {
  try {
    return await prisma.review.findMany({
      where: { revieweeId, isAutoGenerated: false },
      include: {
...
async function loadPresentationStats(userId: number) {
  try {
    ...
    return computePresentationStats(appointments);
  } catch {
    return { held: 0, noShow: 0, scheduled: 0 };
  }
}
