    ...
    void fetch("/api/admin/stats", { cache: "no-store", credentials: "include" })
      ...
      .then((data) => {
        ...
        if (typeof console !== "undefined" && console.info) {
          console.info(`[centrala/statystyki] loaded in ${Math.round(performance.now() - started)}ms`);
        }
        setStats(data);
      })
      .catch((err) => {
        if (cancelled) return;
        setLoadError(err instanceof Error ? err.message : "Błąd ładowania");
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="theme-aware-dashboard min-h-screen bg-[var(--eos-bg)] px-4 pb-16 pt-28 text-[var(--eos-text)] sm:px-6 md:px-10 md:pt-32">
      <div className="mx-auto max-w-[1400px]">
        ...
        {loadError ? (
          <div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-5 py-8 text-center text-sm text-red-700 dark:text-red-200">
            Nie udało się wczytać analityki: {loadError}
          </div>
        ) : !stats ? (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-[var(--eos-border)] bg-[var(--eos-card)] px-6 py-24">
            <Loader2 className="size-8 animate-spin text-emerald-500" />
            <p className="text-xs font-semibold uppercase tracking-widest text-[var(--eos-muted)]">
              Ładowanie analityki…
            </p>
            <p className="max-w-sm text-center text-[11px] text-[var(--eos-subtle)]">
              KPI, wizyty i rynek ładują się jednym zapytaniem — po optymalizacji powinno być zauważalnie szybciej.
            </p>
          </div>
        ) : (
          <>
              ...
              {[
                ...
                <div key={kpi.label} className="rounded-xl border border-[var(--eos-border)] bg-[var(--eos-card)] px-3 py-2.5">
                  ...
                  <p className="text-xl font-black tabular-nums">
                    {typeof kpi.value === "number" ? Number(kpi.value).toLocaleString("pl-PL") : kpi.value}
                  </p>
                </div>
              ))}
            </div>
