...
console.log("=== AKTUALIZACJA PREMIUM MODE TOGGLE (APPLE STYLE) ===");
...
try {
  ...
  // Wymiana całego kontenera wraz z 3 przyciskami na nową strukturę
  const newStructure = `
...
`;
  ...
  // Używamy matchowania na starym kontenerze (od `<div className="flex flex-row gap-2` aż do końca div-a)
  const regex = /\{\/\*\s*KONTENER PRZYCISKÓW\s*\*\/\}[\s\S]*?<\/div>\s*<\/div>/;
  
  if (regex.test(code)) {
      code = code.replace(regex, newStructure + '\n    </div>');
      fs.writeFileSync(togglePath, code);
      console.log("✅ Wstrzyknięto luksusowy przełącznik Apple Glassmorphism z diodami statusu.");
  } else {
      console.log("⚠️ Nie udało się zlokalizować oryginalnego bloku przycisków w pliku.");
  }
