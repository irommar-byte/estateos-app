...
o(f"\t\t{PROD_GRP} = {{isa = PBXGroup; children = ({APP_REF} /* ParagonOS.app */, {TEST_REF} /* ParagonOSTests.xctest */); name = Products; sourceTree = \"<group>\"; }};")
...
o(f"\t\t{MAIN_GRP} = {{isa = PBXGroup; children = ({APP_GRP} /* ParagonOS */, {TEST_GRP} /* ParagonOSTests */, {PROD_GRP} /* Products */); sourceTree = \"<group>\"; }};")
...
tgt_settings = (
    ...
    f"CODE_SIGN_STYLE = Automatic; CURRENT_PROJECT_VERSION = {BUILD_NUMBER}; VERSIONING_SYSTEM = \"apple-generic\"; "
    ...
    "MARKETING_VERSION = 1.0.0; PRODUCT_BUNDLE_IDENTIFIER = pl.paragonos.app; "
...
print(f"Generated {OUT} with {len(SWIFT)} Swift files + {len(TEST_SWIFT)} tests + {len(RES)} resources")
