...
console.log("=== ROZPOCZYNAM TWORZENIE UNIWERSALNEGO SILNIKA MODALI ===");
...
const modalPath = path.join(uiDir, 'BaseModal.tsx');
...
export default function BaseModal({ isOpen, onClose, children, title, maxWidth = "max-w-2xl" }: BaseModalProps) {
  ...
  return (
    ...
    <div 
      className="fixed inset-0 z-[99999] flex items-start justify-center overflow-y-auto bg-black/60 backdrop-blur-sm p-4 sm:p-6 transition-opacity" 
      onClick={onClose}
    >
      {/* Kontener modala - items-start w rodzicu i my-auto tutaj zapobiega ucinaniu od góry przy wysokich modalach */}
      <div 
        className={\`relative w-full \${maxWidth} bg-white rounded-2xl shadow-2xl my-auto transform transition-all flex flex-col max-h-[90vh]\`} 
...
try {
  ...
  console.log("✅ Utworzono niezawodny silnik modali: src/components/ui/BaseModal.tsx");
} catch (e) {
  console.error("❌ Błąd tworzenia pliku:", e.message);
}

console.log("\n=== SKANOWANIE APLIKACJI W POSZUKIWANIU STARYCH MODALI ===");
